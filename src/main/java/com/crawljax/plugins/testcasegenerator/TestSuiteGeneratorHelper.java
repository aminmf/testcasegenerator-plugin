...
public class TestSuiteGeneratorHelper {
	private static final Logger LOGGER = Logger.getLogger(TestSuiteGeneratorHelper.class
	        .getName());
	...
	public void writeEventableTestDataToXML(String fname) throws IOException {
		...
		for (Eventable eventable : eventables) {
			...
			element.setTag(eventable.getElement().getTag());
			element.setText(eventable.getElement().getText());
			List<Attribute> attributes = new ArrayList<Attribute>();
			for (Attribute attribute : eventable.getElement().getAttributes()) {
				attributes.add(attribute);
			}
			element.setAttributes(attributes);
			// element.setAttributes(eventable.getElement().getAttributes());
